		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
			...
			if (process.platform === "win32") {
				...
				try {
					...
					const displayBounds = source?.id?.startsWith("window:")
						? null
			...
			try {
				...
				if (
					...
					appName &&
					(appName === ownAppName || appName === "unbound")
				) {
					return {
						success: false,
						message: "Cannot record Unbound windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message:
							"Unbound needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure Unbound is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "Unbound needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure Unbound is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
